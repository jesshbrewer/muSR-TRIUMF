...
MANGLE(void,fMUD_close,FMUD_CLOSE,fmud_close)( pFio )
    FILE** pFio;
{
    fclose( *pFio );
}
...
MANGLE(void,fMUD_rewind,FMUD_REWIND,fmud_rewind)( pFio )
    FILE** pFio;
{
    rewind( *pFio );
}
...
MANGLE(void,fMUD_freeOne,FMUD_FREEONE,fmud_freeone)( pMUD )
    MUD_SEC* pMUD;
{
    if( pMUD == NULL ) return;
...
MANGLE(void,fMUD_freeAll,FMUD_FREEALL,fmud_freeall)( ppMUD_head )
    MUD_SEC** ppMUD_head;
{
    MUD_free( *ppMUD_head );
}
...
MANGLE(int,fMUD_writeEnd,FMUD_WRITEEND,fmud_writeend)( pFout )
    FILE** pFout;
{
...
MANGLE(int,fMUD_write,FMUD_WRITE,fmud_write)( pFout, pMUD )
    FILE** pFout;
    MUD_SEC* pMUD;
{
...
MANGLE(int,fMUD_writeGrpStart,FMUD_WRITEGRPSTART,fmud_writegrpstart)
    ( pFout, pMUD_parentGrp, pMUD_grp, pNumMems )
    FILE** pFout;
...
MANGLE(MUD_SEC*,fMUD_readFile,FMUD_READFILE,fmud_readfile)( pFin )
    FILE** pFin;
{
...
MANGLE(int,fMUD_read,FMUD_READ,fmud_read)( pFin, pMUD_ret )
    FILE** pFin;
    MUD_SEC* pMUD_ret;
{
    MUD_SEC* pMUD_new;
