 ...
 *
 *   Copyright (C) 1994-2001 TRIUMF (Vancouver, Canada)
 *
...
void 
mu_interp_time( TIME* ptimval_out, char in_date[], char in_time[] )
{
    ...
    char* exptarea;
{
     ...
     *  Assemble time info
     */
    mu_interp_time( &pMUD_desc->timeBegin, 
		    &pImusr_hdr->dattimstart[0], &pImusr_hdr->dattimstart[10] );
    mu_interp_time( &pMUD_desc->timeEnd, 
    ...
    MUD_SEC_GRP* pMUD_indVarGrp;
{
	...
	pMUD_indVarDat->pTime = NULL;
    	...
    	if( pImusr_hdr->nvers >= 9 )
        {
          pMUD_indVarHdr->name = strndup_priv( imusr_camp_blk.var[i].path,
                                          (size_t)imusr_camp_blk.var[i].len_path );
          pMUD_indVarHdr->description = strndup_priv( imusr_camp_blk.var[i].title,
                                          (size_t)imusr_camp_blk.var[i].len_title );
          pMUD_indVarHdr->units = strndup_priv( imusr_camp_blk.var[i].units,
                                          (size_t)imusr_camp_blk.var[i].len_units );
        }
	else
	{
          pMUD_indVarHdr->name = strndup_priv( imusr_camp_blk_v8.var[i].path,
                                          (size_t)imusr_camp_blk_v8.var[i].len_path );
          pMUD_indVarHdr->description = strndup_priv( imusr_camp_blk_v8.var[i].title,
                                          (size_t)imusr_camp_blk_v8.var[i].len_title );
          pMUD_indVarHdr->units = strndup_priv( imusr_camp_blk_v8.var[i].units,
                                          (size_t)imusr_camp_blk_v8.var[i].len_units );
	}
