    ...
    IMUSR_HDR* pImusr_hdr;
{
     ...
     *  Assemble time info
     */
    pTm_time = localtime( (time_t*)&pMUD_desc->timeBegin );
    sprintf( pImusr_hdr->dattimstart, "%02d%3s19%02d %02d:%02d:%02d",
	     pTm_time->tm_mday, getMonthString( pTm_time->tm_mon ), 
	     pTm_time->tm_year, 
	     pTm_time->tm_hour, pTm_time->tm_min, pTm_time->tm_sec ); 
    pImusr_hdr->lendatestrt = strlen( pImusr_hdr->dattimstart );

    pTm_time = localtime( (time_t*)&pMUD_desc->timeEnd );
    sprintf( pImusr_hdr->dattimeend, "%02d%3s19%02d %02d:%02d:%02d",
	     pTm_time->tm_mday, getMonthString( pTm_time->tm_mon ), 
	     pTm_time->tm_year, 
	     pTm_time->tm_hour, pTm_time->tm_min, pTm_time->tm_sec ); 
