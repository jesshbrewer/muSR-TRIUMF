...
int MUD2TRI_convert _ANSI_ARGS_(( char* inFile, char* outFile ));
...
int MUD2COL_convert _ANSI_ARGS_(( char* fin_name, char* fout_name ));
...
usage()
{
    ...
    printf( "\t-e<expt#>    -  experiment number (a single integer)\n" );
    printf( "\t-m<members>  -  experiment members (a string with no spaces)\n" );
    printf( "\t-a<area>     -  beamline or experimental area\n" );
    ...
    char* argv[];
{
  ...
  else if( !strncmp( command, "mud2col", 7 ) )
  {
      ...
      MUD2COL_convert( infile, outfile );
  }
