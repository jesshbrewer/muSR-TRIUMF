...
static int MUD2TRI_hists  _ANSI_ARGS_(( FILE *fout , TMF_F_HDR *pTmf_f_hdr , MUD_SEC_GRP *pMUD_histGrp ));

static UINT32 TRI2MUD_BinSizes[32] =
    {    78125,   156250,   312500,    625000,   1250000,   2500000,    5000000,   10000000,
      20000000, 40000000, 80000000, 160000000, 320000000, 640000000, 1280000000, 2560000000,
...
int
MUD2TRI_convert( inFile, outFile )
    char* inFile;
    char* outFile;
{
    int status;
     ...
     */
    status = MUD2TRI_hists( fout, &tmf_f_hdr, pMUD_histGrp );
    if( _failure( status ) )
    {
	goto error;
    }
...
static int
MUD2TRI_hists( fout, pTmf_f_hdr, pMUD_histGrp )
    FILE* fout;
    TMF_F_HDR* pTmf_f_hdr;
    MUD_SEC_GRP* pMUD_histGrp;
{
